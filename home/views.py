...
def project_specific(request, project_id):
    ...
    try:
        ...
        messages.warning(request, 'No hay proyecto para este ID')
...
def contact(request):
    ...
    if request.method =='POST':
        ...
        if username == '' or email == '':
            messages.warning(request, 'Por favor complete el Nombre y el ID de correo electrónico')
            return redirect('contact')
        else:
            messages.success(request, 'Recibimos su correo electrónico y le responderemos en breve!')
